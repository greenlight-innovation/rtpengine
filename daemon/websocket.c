...
typedef const char *(*websocket_message_func_t)(struct websocket_conn *, struct websocket_message *);
...
static struct websocket_message *websocket_message_new(void) {
...
static int websocket_write_raw(struct websocket_conn *wc, const char *msg, size_t len, enum lws_write_protocol protocol) {
	mutex_lock(&wc->lock);
...
INLINE int websocket_write_http(struct websocket_conn *wc, const char *msg) {
	return websocket_write_raw(wc, msg, strlen(msg), LWS_WRITE_HTTP);
}
INLINE int websocket_write_binary(struct websocket_conn *wc, const char *msg, size_t len) {
...
static const char *websocket_do_ping(struct websocket_conn *wc, struct websocket_message *wm) {
	ilog(LOG_DEBUG, "Respoding to GET /ping");

	uint8_t buf[LWS_PRE + 2048], *start = &buf[LWS_PRE], *p = start,
		*end = &buf[sizeof(buf) - LWS_PRE - 1];

	if (lws_add_http_header_status(wc->wsi, 200, &p, end))
	...
	if (lws_add_http_header_content_length(wc->wsi, 5, &p, end))
	...
	if (websocket_write_http(wc, "pong\n"))
...
static int websocket_http_ping(struct websocket_conn *wc) {
	websocket_message_push(wc, websocket_do_ping);
	return 0;
}
...
static int websocket_http_get(struct websocket_conn *wc) {
	...
	ilog(LOG_DEBUG, "HTTP GET %s", wm->uri);
	wc->wm->method = M_GET;

	if (!strcmp(wc->wm->uri, "/ping"))
		return websocket_http_ping(wc);

	ilog(LOG_WARN, "Unhandled HTTP GET URI: '%s'", wm->uri);
	return -1;
}
